...
export default function AdminLayout() {
  ...
  // Single-institute site (SMS): always show SMS's own menu tree directly,
  ...
  if (isAdmin) {
    ...
    const smsMenuFiltered = (menuByDirectorate['sms'] || []).filter(
      (item) => item.menuKey !== 'online-resources' && item.parentKey !== 'online-resources'
    );
    const smsItems = [
      ...(smsMenuFiltered.length
        ? [
            ...smsMenuFiltered.map((item) => ({
              to: menuItemHref(item, 'sms'),
              label: `${'\u2003'.repeat(item.depth)}${item.label}`,
              icon: item.children.length ? 'fa-folder' : (TYPE_ICON[item.type] || 'fa-file'),
              onClick: close,
            })),
            { to: `/admin/r/directorate-menu?directorate=sms`, label: 'Manage menu items', icon: 'fa-sliders', onClick: close },
          ]
        : []),
      ...
      // the Institutional group, and SMS doesn't need the rest.
    ];
    groups.push({ label: 'SMS Site Pages', items: smsItems });
  }
  ...
  const nav = (
    <Sidebar
      header={<Link to="/admin"><span className="block font-display text-lg font-bold text-white">SMS Admin</span><span className="text-xs text-white/60">{isAdmin ? 'Admin' : (user?.directorate || 'Staff')}</span></Link>}
      groups={groups}
      footer={<>
    ...
    <div className="min-h-screen bg-canvas text-content"><NavProgress />
      ...
      <div className="md:pl-64">
        <header className="flex items-center gap-3 border-b border-line bg-surface px-4 py-3 md:hidden">
          ...
          <span className="font-display font-bold text-navy">SMS Admin</span>
        </header>
