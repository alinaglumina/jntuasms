import { useQuery } from '@tanstack/react-query';
import { downloadsQuery } from '../api/queries.js';

// Guest Lectures grouped by year (derived from each entry's Date field):
// a year heading, then that year's titles listed below with S.No + Download.
export default function GuestLecturesTable() {
  const { data, isLoading } = useQuery(downloadsQuery('guest-lectures'));
  const items = data?.items || [];

  if (isLoading) return <div className="skeleton h-40 w-full rounded-lg" />;
  if (items.length === 0) return <p className="rounded-md border border-dashed border-slate-300 p-8 text-center text-sm text-slate-500">No guest lectures listed yet.</p>;

  const groups = {};
  items.forEach((it) => {
    const year = it.date ? new Date(it.date).getFullYear() : 'Undated';
    (groups[year] ||= []).push(it);
  });
  const sortedYears = Object.keys(groups).sort((a, b) => b - a);

  return (
    <div className="space-y-8">
      {sortedYears.map((year) => (
        <div key={year}>
          <h3 className="mb-3 font-display text-xl font-semibold text-navy">{year}</h3>
          <div className="overflow-x-auto rounded-lg border border-line shadow-card">
            <table className="w-full text-left text-sm">
              <thead className="bg-navy/5 text-navy">
                <tr>
                  <th className="px-4 py-3 w-16">S.No</th>
                  <th className="px-4 py-3">Title</th>
                  <th className="px-4 py-3 w-28 text-center">Download</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-line">
                {groups[year].map((doc, i) => (
                  <tr key={doc._id} className={i % 2 === 0 ? 'bg-white' : 'bg-navy/[0.02]'}>
                    <td className="px-4 py-3 text-slate-500">{String(i + 1).padStart(2, '0')}.</td>
                    <td className="px-4 py-3 text-slate-700">{doc.title}</td>
                    <td className="px-4 py-3 text-center">
                      {doc.attachment ? (
                        <a href={doc.attachment} target="_blank" rel="noopener noreferrer" aria-label={`Download ${doc.title}`}
                           className="inline-grid h-8 w-8 place-items-center rounded bg-crimson/10 text-crimson hover:bg-crimson hover:text-white">
                          <i className="fa-solid fa-file-pdf" aria-hidden="true" />
                        </a>
                      ) : <span className="text-slate-300">—</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        </div>
      ))}
    </div>
  );
}
